  ...
  <section class="cart-page">
    <article class="cart-list dm-card">
      <header class="cart-list__header">
        <div><h1>{{ Text.Title }}</h1><p>{{ formatI18nText(Text.SelectedCount, { count: SelectedQuantity }) }}</p></div>
      ...
      <div v-if="CartItems.length" class="cart-items">
        <article v-for="CartItem in CartItems" :key="CartItem.CartItemId" class="cart-item">
          <input type="checkbox" :checked="SelectedItemIds.includes(CartItem.CartItemId)" :aria-label="formatI18nText(Text.SelectProductLabel, { name: CartItem.ProductName })" @change="toggleItem(CartItem.CartItemId, $event.target.checked)" />
          ...
          <div class="cart-item__information">
            ...
            <span>{{ formatI18nText(Text.ProductCodeValue, { code: CartItem.VariantName }) }}</span>
          ...
          <div class="cart-quantity">
            ...
            <small>{{ formatI18nText(Text.AvailableStock, { count: CartItem.AvailableStock }) }}</small>
          </div>
          <strong class="cart-item__price">{{ formatCurrency(CartItem.LineTotal) }}</strong>
          <DMButton type="none" :is-tooltip="false" class="cart-item__remove" icon-name="delete" :aria-label="formatI18nText(Text.RemoveProductLabel, { name: CartItem.ProductName })" @click="removeItem(CartItem.CartItemId)" />
        </article>
      </div>
