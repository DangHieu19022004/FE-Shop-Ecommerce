<template>
  <section class="payment-page">
    ...
    <div v-if="ActionMessage" class="payment-alert payment-alert--success dm-card" role="status">
    ...
    <article class="payment-section payment-address dm-card">
      ...
      <p v-if="HasTriedPlaceOrder && AddressValidationMessage" class="payment-alert payment-alert--danger" style="margin-top: 12px;">{{ AddressValidationMessage }}</p>
    </article>
    ...
    <article class="payment-section payment-extras dm-card">
      <div class="payment-voucher">
        ...
        <p v-if="HasTriedPlaceOrder && VoucherValidationMessage" class="payment-alert payment-alert--danger" style="margin: 12px 0 0;">{{ VoucherValidationMessage }}</p>
          ...
          <div v-if="VisibleValidationMessages.length" class="payment-alert payment-alert--danger dm-card" role="alert" style="margin-bottom: 12px;">
            ...
            <div>
              ...
              <ul style="margin: 8px 0 0 18px; padding: 0;">
                <li v-for="Message in VisibleValidationMessages" :key="Message">{{ Message }}</li>
              </ul>
            </div>
          </div>
          <DMButton type="none" :is-tooltip="false" class="payment-summary__submit" icon-name="shopping_bag" :message="IsSubmitting ? 'Đang đặt hàng...' : Text.PlaceOrder" :un-active="IsSubmitting" @click="placeOrder" />
          <p>{{ Text.TermsNotice }}</p>
        </div>
      </div>
    </aside>
  </section>
...
const ValidationMessages = computed(() => {
  ...
  if (!Addresses.value.length) {
    Messages.push("Thiếu địa chỉ nhận hàng. Vào hồ sơ để thêm địa chỉ trước khi đặt hàng.");
  } else if (!SelectedAddressId.value) {
    Messages.push("Cần chọn địa chỉ nhận hàng.");
  }

  if (!CheckoutItems.value.length || MerchandiseSubtotal.value <= 0) {
    Messages.push("Giỏ hàng đang trống. Hãy thêm sản phẩm trước khi đặt hàng.");
  }

  if (SelectedVoucher.value && !IsVoucherEligible.value) {
    Messages.push(formatI18nText(Text.VoucherMinimumRequired, { amount: formatCurrency(SelectedVoucher.value.MinimumOrderAmount) }));
  }

  return Messages;
});
const AddressValidationMessage = computed(() => {
  if (!Addresses.value.length) {
    return "Thiếu địa chỉ nhận hàng. Vào hồ sơ để thêm địa chỉ trước khi checkout.";
  }

  if (!SelectedAddressId.value) {
    return "Cần chọn địa chỉ nhận hàng.";
  }

  return "";
});
const VoucherValidationMessage = computed(() => {
  if (SelectedVoucher.value && !IsVoucherEligible.value) {
    return formatI18nText(Text.VoucherMinimumRequired, { amount: formatCurrency(SelectedVoucher.value.MinimumOrderAmount) });
  }

  return "";
});
const HasTriedPlaceOrder = ref(false);
const VisibleValidationMessages = computed(() => (HasTriedPlaceOrder.value ? ValidationMessages.value : []));

const resetCheckoutState = () => {
  HasTriedPlaceOrder.value = false;
...
const placeOrder = async () => {
  HasTriedPlaceOrder.value = true;
  ErrorMessage.value = "";
  SuccessMessage.value = "";
  ActionMessage.value = "";

  if (ValidationMessages.value.length) {
    ErrorMessage.value = "Vui lòng bổ sung thông tin còn thiếu trước khi đặt hàng.";
    return;
  }
  ...
  try {
    ...
    const OrderCode = Result.OrderCode || Result.Order?.OrderCode || "";
    SuccessMessage.value = formatI18nText(Text.OrderSuccessWithCode, { code: OrderCode });
    resetCheckoutState();
    await loadCart();

    if (Result?.Payment?.QrImageUrl) {
      ActionMessage.value = "Đơn đã tạo. Mở chi tiết đơn để quét QR Sepay và theo dõi trạng thái.";
    }

    if (OrderCode) {
      await Router.push({ name: "orderDetail", params: { orderCode: OrderCode } });
    }
  } catch (Error) {
    ErrorMessage.value = Error.message;
  } finally {
